...
const AdminDashboard = () => {
  ...
  const empRes = await fetch("/api/employees/pending");
    const empData = await empRes.json();

  const hrRes = await fetch("/api/hr/pending");
  ...
  await fetch(`/api/employees/approve/${data.id}`, { method: "POST" });
      fetchEmployees();
      fetchSignupRequests();
  ...
  const response = await fetch("/api/hr/create", {
    ...
    if (response.ok) {
      const data = await response.json();
  await fetch(`/api/hr/approve/${data.id}`, { method: "POST" });
      fetchHrAccounts();
      fetchSignupRequests();
      setShowAddHr(false);
      setNewHr({
  ...
  url = `/api/employees/approve/${request.id}`;
    } else {
  url = `/api/hr/approve/${request.id}`;
    }
  ...
  const handleRejectRequest = async (request) => {
    let url = "";
    if (request.role === "employee") {
  url = `/api/employees/reject/${request.id}`;
    } else {
  url = `/api/hr/reject/${request.id}`;
    }
